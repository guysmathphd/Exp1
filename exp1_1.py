...
def exp1_1_main(input_file_path, input_file_name, recalc_flag, redraw_flag, first_scenario, last_scenario):
    input_struct = read_input(input_file_path + input_file_name)

    if recalc_flag:
        for i in range(first_scenario, last_scenario+1):
            print('Running scenario ' + str(i))
            run_scenario(input_file_path, input_struct.row_values(i))
            print('Finished running scenario ' + str(i))

    if redraw_flag:
        for i in range(first_scenario, last_scenario+1):
            print('Plotting scenario  ' + str(i))
            plot_scenario(input_file_path, input_struct.row_values(i))
            print('Finished plotting scneario ' + str(i))
    return


input_file_path_str = 'C:\\Users\\Guy\\PycharmProjects\\Exp1Scenarios3\\'
input_file_name = 'Exp1-Input.xlsx'
exp1_1_main(input_file_path_str, input_file_name, 1, 1, 28, 28)
# from rkf45 import *
# def my_cos(t, y):
